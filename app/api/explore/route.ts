...
export async function GET() {
  try {
    ...
    // Fetch random Users - all users (could filter by role if needed)
    ...
    // Get random samples from each category (adjust counts as needed)
    ...
    const transformedVocab = randomVocab.map(vocab => ({
      ...
      difficulty: 'BEGINNER' as const, // Default, could be calculated
      rating: 4.5, // Default, could be calculated from reviews
      totalLearners: Math.floor(Math.random() * 1000) + 100, // Mock, could be calculated
    }));
